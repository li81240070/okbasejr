...
public class LoginOrderPage extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view=LayoutInflater.from(getContext()).inflate(R.layout.loginorderpage,null);
...
//与接收系统的一样
        ...
        loginOrderListView= (PostCardList) view.findViewById(R.id.loginOrderListView);
        ...
        NetLoader.getInstance(getContext()).loadGetData(getActivity(), url, new NetLoader.NetResponseListener() {
            ...
            public void success(String resultString, int code) {
                ...
                for (int i = 0; i < orderClass.getDataMap().getOrders().size(); i++) {
                    if (orderClass.getDataMap().getOrders().get(i).getOrder_status().equals("2")||
                            orderClass.getDataMap().getOrders().get(i).getOrder_status().equals("3")||
                            orderClass.getDataMap().getOrders().get(i).getOrder_status().equals("4")
                            ){
                ...
                loginOrderListView.setAdapter(adapter);
