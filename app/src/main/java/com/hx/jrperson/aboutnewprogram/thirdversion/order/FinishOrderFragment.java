...
public class FinishOrderFragment extends Fragment{
    ...
    private PostCardList finishOrderListView;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view=LayoutInflater.from(getContext()).inflate(R.layout.finishorderpage,null);
...
//与接收系统的一样
        ...
        finishOrderListView= (PostCardList) view.findViewById(R.id.finishOrderListView);
        ...
        NetLoader.getInstance(getContext()).loadGetData(getActivity(), url, new NetLoader.NetResponseListener() {
            ...
            public void success(String resultString, int code) {
                ...
                for (int i = 0; i < orderClass.getDataMap().getOrders().size(); i++) {
                    if (orderClass.getDataMap().getOrders().get(i).getOrder_status().equals("99") ){
                        data.add(orderClass.getDataMap().getOrders().get(i));
                    }

                }
                adapter.setData(data);
                finishOrderListView.setAdapter(adapter);
